...
// #define SND_STREAM_BUFFER_MAX_ADPCM 8
// #define SND_STREAM_MAX 128
...
int wav_init(void)
{
	...
	if (snd_stream_init_ex(1, SND_STREAM_BUFFER_MAX_ADPCM) < 0)
		return 0;
...
wav_stream_hnd_t wav_create(const char *filename, int loop)
{
	...
	index = snd_stream_alloc(wav_file_callback, SND_STREAM_BUFFER_MAX_ADPCM);
	...
	if (fn_len >= 4 && ((strcmp(&filename[fn_len - 4], ".raw") == 0) || (strcmp(&filename[fn_len - 4], ".pcm") == 0))) {
	...
	streams[index].drv_buf = memalign(32, SND_STREAM_BUFFER_MAX_ADPCM);
...
void wav_play(wav_stream_hnd_t hnd)
{
	...
	printf("wav_play(%d) OK\n", hnd);
	printf("streams[%d].status = %d\n", hnd, streams[hnd].status);
}

void wav_pause(wav_stream_hnd_t hnd)
{
	if (streams[hnd].status == SNDDEC_STATUS_READY || streams[hnd].status == SNDDEC_STATUS_PAUSING)
		return;

	streams[hnd].status = SNDDEC_STATUS_PAUSING;
	printf("wav_pause(%d) OK\n", hnd);
	printf("streams[%d].status = %d\n", hnd, streams[hnd].status);
}

void wav_stop(wav_stream_hnd_t hnd)
{
	if (streams[hnd].status == SNDDEC_STATUS_READY || streams[hnd].status == SNDDEC_STATUS_STOPPING)
		return;

	streams[hnd].status = SNDDEC_STATUS_STOPPING;
	printf("wav_stop(%d) OK\n", hnd);
	printf("streams[%d].status = %d\n", hnd, streams[hnd].status);
}
...
void wav_volume(wav_stream_hnd_t hnd, int vol)
{
	...
	printf("wav_volume(%d, %d) OK\n", hnd, vol);
	printf("streams[%d].vol = %d\n", hnd, streams[hnd].vol);
}

int wav_is_playing(wav_stream_hnd_t hnd)
{
	printf("wav_is_playing(%d) = %d\n", hnd, streams[hnd].status == SNDDEC_STATUS_STREAMING);
...
static void *sndwav_thread(void *param)
{
	...
	// printf("sndwav_thread\n");
	while (sndwav_status != SNDDRV_STATUS_DONE) {

		mutex_lock(&stream_mutex);

		for (i = 0; i < SND_STREAM_MAX; i++) {
			// printf("    before: streams[%d].status = %d\n", i, streams[i].status);
			switch (streams[i].status) {
			case SNDDEC_STATUS_RESUMING:
				printf("    before: streams[%d].status = %d\n", i, streams[i].status);
				printf("          : streams[%d].sample_size = %d\n", i, streams[i].sample_size);
				if (streams[i].sample_size == 16) {
					...
					printf("snd_stream_start_adpcm(%d, %d, %d)\n", streams[i].shnd, streams[i].sample_rate, streams[i].channels - 1);
				...
				printf("    after: streams[%d].status = %d\n", i, streams[i].status);
				break;
